...
import { handsHoldingRakhi, mirrorHandsForCanvas, normalizeHands, rakhiPlacement, restoreHandsToCanvas } from '../src/rakhi/handRetargeting';
...
describe('hand normalization / retargeting', () => {
  it('normalizes palm geometry into 21 local joints', () => {
  ...
  it('keeps physical distances stable across camera aspect ratios', () => {
    const wide = normalizeHands([makeHand(.35), makeHand(.65)], 16 / 9);
    expect(wide[0].aspect).toBeCloseTo(16 / 9);
    const placement = rakhiPlacement(wide);
    expect(placement?.span).toBeGreaterThan(.3);
    const restored = restoreHandsToCanvas(wide);
    expect(restored[0][0].x).toBeCloseTo(.35, 4);
  });
